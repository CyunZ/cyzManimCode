...
def getSongFormula2(wavfile_name,filter_amplitude=50,max_fre = None):
    """
    返回音频的余弦函数表达式参数(精准版)
        wavfile_name:文件名
        filter_amplitude:过滤掉该强度以下的频率
    ...
    if max_fre is None:
        ...
        max_fre = Fs / 2
    params = []
    dc = abs_yy[0] /N # 直流分量 被放大了N倍

    for i in range(1,len(abs_yy)):
        freq = i*Fs/N   # 频率 = index * Fs / N
        if freq > max_fre: # 最大频率不会超过采样频率的一般
            break
        amplitude = abs_yy[i] * 2.0/N
        if amplitude > filter_amplitude: # 过滤幅值低于指定值的频率信号
                phase =  np.angle( yy[i],True ) # 计算相位
